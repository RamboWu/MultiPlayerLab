...
APlayerProxy::APlayerProxy(const class FPostConstructInitializeProperties& PCIP)
	: Super(PCIP)
{
...
void APlayerProxy::BeginPlay()
{
	...
	if (Role == ROLE_Authority)
	{
		...
		Character = Cast<AMyProject4Character>(GetWorld()->SpawnActor(CharacterClass, &Location, &Rotation, SpawnParams));
...
void APlayerProxy::MoveToLocation(const AMyProject4PlayerController* controller, const FVector& DestLocation)
{
